defmodule MeetApi.AccountsTest do
  ...
  test "authenticate_user/2 authenticates valid user credentials", %{
    bolt_sips_conn: bolt_sips_conn
  } do
    {:ok, _} = Accounts.create_user(bolt_sips_conn, @valid_attrs)

    assert {:ok, _} =
             Accounts.authenticate_user(bolt_sips_conn, @valid_attrs.email, @valid_attrs.password)
  end
